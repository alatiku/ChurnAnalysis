...
# Churn Analysis on IBM Telco Customer Data[^1]
...
x <- data.frame(miss = colSums(is.na(telco_data)), 
             prop = round((colSums(is.na(telco_data)) / nrow(telco_data)) * 100, 
                           2))
...
```{r churn_map, message=FALSE, warning=FALSE}
# Filter the dataset to include only churned customers
churned_customers <- telco_data[telco_data$`Churn Label` == "Yes",]
leaflet(data = churned_customers) %>%
    addTiles() %>%  # Add default OpenStreetMap tiles as basemap
    addCircleMarkers(
        ~Longitude, ~Latitude,  # Specify Longitude and Latitude columns
        radius = 1,  # Adjust the radius of the bubbles as needed
        color = "red",  # Set color of the bubbles to red
        fillOpacity = 0.2,  # Adjust the opacity of the bubbles
        # Display in popup
        ...
        # Display CustomerID as label on markers
        label = ~CustomerID) %>%
    addLegend(
        "bottomleft",  # Position of the legend
        colors = "red",  # Color for the legend
        labels = "Churned",  # Label for the legend
        title = "Churn Label"  # Title for the legend
    )
```

### What is the distribution of churn among all customers?

```{r churn_analysis1, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
# Churn Distribution
...
`r churn_rate[2, 2]` customers have churned resulting in a churn rate of approximately `r churn_rate[2, 3]`%, which is considerably high.
...
```{r churn_analysis2, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
```{r churn_analysis3, message=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
```{r churn_analysis4, message=FALSE, warning=FALSE, fig.height=12, fig.width=12}
...
```{r churn_analysis5, message=FALSE, warning=FALSE, fig.height=6, fig.width=6}
...
1.  The analysis couldn't explore customer reasons for churning due to the high proportion of missing values for that variable, and as such, the insights into the specific motivations or pain points driving churn remain limited. This missing information could potentially lead to an incomplete understanding of the underlying factors contributing to customer attrition and might hinder the development of targeted strategies to address these issues effectively.

2.  The data does not consider temporal trends that could affect churn rates over time. This is especially important because customer preferences and behaviors could change over time, so conclusions drawn from historical data might not be applicable to the future.

3.  Logistic regression and decision tree models make certain assumptions about data distribution and relationships, and any violation of these assumptions could affect the validity of the analysis.
